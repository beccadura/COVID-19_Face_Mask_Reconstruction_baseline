...
if is_directory:
    ...
    for f in tqdm(files):
        ...
        write_path_binary = path + "_binary"
        if not os.path.isdir(write_path):
            os.makedirs(write_path)
        if not os.path.isdir(write_path_binary):
            os.makedirs(write_path_binary)
        ...
        if is_image(image_path):
            ...
            for i in range(len(mask)):
                ...
                b_path = write_path_binary + "/" + split_path[0] + "_binary." + split_path[1]
                binary_img = mask_binary_array[i]
                cv2.imwrite(b_path, binary_img)
        ...
        for f in files:
            ...
            if is_image(image_path):
                ...
                for i in range(len(mask)):
                    ...
                    # Write the binary image
                    b_path = write_path + "/" + split_path[0] + "_binary." + split_path[1]
                    binary_img = mask_binary_array[i]
                    cv2.imwrite(b_path, binary_img)
...
# Process if the path was a file
    ...
    if is_image(image_path):
        ...
        for i in range(len(mask)):
            w_path = write_path + "_masked." + args.path.rsplit(".")[1]
            img = masked_image[i]
            cv2.imwrite(w_path, img)
            # Write the binary image
            b_path = write_path + "_binary." + args.path.rsplit(".")[1]
            binary_img = mask_binary_array[i]
            cv2.imwrite(b_path, binary_img)
