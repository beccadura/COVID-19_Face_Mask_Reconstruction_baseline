...
class Reconstruction_Trainer():
    def __init__(self, args, cfg):
        ...
        self.num_iters = (self.num_epochs) * len(self.trainloader)
        ...
        try:
            for epoch in range(self.epoch, self.num_epochs):
                ...
                for i, batch in enumerate(self.trainloader):
                    ...
                    image_output = self.model_G(imgs, masks)
    
                    whole_image_output = imgs * (1 - masks) + image_output * masks

                    masks = masks.cpu()

                    fake_D = self.model_D(whole_image_output.detach())
                    real_D = self.model_D(ground_truth)
                    ...
                    loss = self.cfg.lambda_rc * loss_rc + self.cfg.lambda_per * loss_P
                    loss.backward()
                    ...
                    if self.iters % self.visualize_per_iter == 0:
                        ...
                        img_list = [imgs, masked_imgs, image_output, whole_image_output, ground_truth]
